...
# Scaling-sweep knob: if QUEUE_SIZE is set, generate an inline config
# that caps SLURM in-flight tasks at $QUEUE_SIZE. Used by
# submit_PXD071075_sweep.sh / run_PXD071075_scaling.sh.
#
# IMPORTANT: write a FULL executor-block replacement (not a thin
# `executor { queueSize = N }` snippet). The pride_slurm profile's
# executor block contains a `jobName = { ... }` closure, and Nextflow's
# merge of a thin override silently falls through to the profile value
# for queueSize in that case. By restating every non-closure setting
# the profile defines, this -c override unambiguously wins.
#
# Keep the non-queueSize values byte-for-byte in sync with
# quantmsdiann/conf/pride_codon_slurm.config so we don't drift.
...
EOF
    ...
    echo "                 [override pride_slurm executor.queueSize=2000]"
fi
...
# Default to -resume (cheap recovery from transient failures), but skip
# it when QUEUE_SIZE is set — a scaling-sweep point must measure a
# fresh end-to-end run, not a resume of an unrelated prior attempt.
RESUME_ARGS=( -resume )
if [ -n "${QUEUE_SIZE:-}" ]; then
    RESUME_ARGS=()
    echo "Resume         : disabled for sweep point (QUEUE_SIZE is set)"
fi
